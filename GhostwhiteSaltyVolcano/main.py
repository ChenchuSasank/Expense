...
def submit_record():
    ...
    if category == 'Income':
        ...
        # All other categories are expense categories
...
def manage_records():
    ...
    # Filter indexed records by category type (for Expense, show all expense categories)
...
def add_subcategory():
    ...
    expense_subcategory = request.form.get('expense_subcategory', '').strip()
    ...
    # For Income categories, add directly to Income list
    if category_type == 'Income':
        ...
        if 'Income' not in subcategories:
            subcategories['Income'] = []

        subcategories['Income'].append({
            'name': subcategory_name,
            'icon': subcategory_icon
        })

    # For Expense categories, create a new main category with its subcategory
    elif category_type == 'Expense':
        if not expense_subcategory:
            return redirect(url_for('categories'))
        ...
        if 'Expense' not in subcategories:
            subcategories['Expense'] = []

        subcategories['Expense'].append({
            'name': subcategory_name,
            'icon': subcategory_icon
        })

        # Also update the expense subcategories mapping
        global expenseSubcategories
        if not hasattr(add_subcategory, 'expenseSubcategories'):
            add_subcategory.expenseSubcategories = {
                'Food': ['Restaurant', 'Fast Food', 'Groceries', 'Street Food', 'Beverages', 'Snacks'],
                'Rent': ['1-bed room flat', '2-bed room flat', '3-bed room flat', 'PG', 'Shop', 'Office Space'],
                'Mobile Recharge': ['Prepaid', 'Postpaid', 'Data Pack', 'International'],
                'Health': ['Doctor Visit', 'Medicines', 'Hospital', 'Dental', 'Eye Care', 'Gym'],
                'Travel': ['Flight', 'Train', 'Bus', 'Taxi/Uber', 'Auto', 'Hotel'],
                'Fuel': ['Petrol', 'Diesel', 'CNG', 'Electric Charging'],
                'Shopping': ['Clothes', 'Electronics', 'Home Items', 'Books', 'Gifts', 'Online Shopping'],
                'Sport': ['Cricket', 'Football', 'Badminton', 'Tennis', 'Swimming', 'Gym Equipment']
            }
        
        # Add the new expense category with its subcategory
        if subcategory_name not in add_subcategory.expenseSubcategories:
            add_subcategory.expenseSubcategories[subcategory_name] = []
        
        if expense_subcategory not in add_subcategory.expenseSubcategories[subcategory_name]:
            add_subcategory.expenseSubcategories[subcategory_name].append(expense_subcategory)
...
@app.route('/get_category_records/<category>')
@require_login()
def get_category_records(category):
    records = load_data()
    # For Income categories, match by Sub-Category
    # For Expense sub-categories, also match by Sub-Category
    filtered = [r for r in records if r['Sub-Category'] == category]
    filtered.sort(key=lambda x: x['Date'], reverse=True)
...
def show_data():
    ...
    subcategory_filter = request.args.get('subcategory_filter', '')
    ...
    for idx, record in enumerate(all_records):
        ...
        if subcategory_filter and record['Sub-Category'] != subcategory_filter:
            continue

        # Filter by date components if specified
        try:
            record_date = datetime.strptime(record['Date'], '%Y-%m-%d')

            if filter_day and record_date.day != int(filter_day):
                continue
            if filter_month and record_date.month != int(filter_month):
                continue
            if filter_year and record_date.year != int(filter_year):
                continue
        except:
            continue
    ...
    expense_records = [r for r in records if r['Category'] != 'Income']
